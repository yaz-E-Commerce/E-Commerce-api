...
const LoginUserDto = z
    .object({
        email: z
            .string({ required_error: 'auth.errors.EMAIL_REQUIRED' })
            .email('auth.errors.INVALID_EMAIL_FORMAT')
            .trim()
            .toLowerCase()
            .openapi({
        ...
        password: z
            .string({ required_error: 'auth.errors.PASSWORD_REQUIRED' })
            .min(6, 'auth.errors.PASSWORD_TOO_SHORT')
            .openapi({
                example: 'StrongPass123',
...
const DeleteUserDto = z.object({
    reason: z
        .string()
        .max(500, 'user.errors.DELETE_REASON_TOO_LONG')
        .optional(),
})
...
const UpdateProfileDto = z.object({
    name: z
        .string()
        .min(2, 'user.errors.NAME_TOO_SHORT')
        .max(100, 'user.errors.NAME_TOO_LONG')
        .optional(),
    gender: z
        .enum(['male', 'female', 'other'], {
